...
4.  **Настройте переменные окружения:**
    Скопируйте файл с примером переменных окружения и при необходимости измените значения.
    ```bash
    cp example.env .env
    ```
    Файл `.env` используется для локальной разработки и уже добавлен в `.gitignore`.
